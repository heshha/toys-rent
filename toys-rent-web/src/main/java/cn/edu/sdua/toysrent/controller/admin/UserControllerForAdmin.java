...
import cn.edu.sdua.toysrent.vo.req.UserUpdateVo;
import cn.edu.sdua.toysrent.vo.resp.OrderRespVo;
import cn.edu.sdua.toysrent.vo.resp.UserRespVo;
...
public class UserControllerForAdmin {
    ...
    public R getOrdersByUserId(@PathVariable("id") long id,
                               @RequestParam(value = "page", defaultValue = "0") int page,
                               @RequestParam(value = "size", defaultValue = "10") int size) {
    ...
    public R updateUser(@RequestBody @Valid UserUpdateVo vo) {
    ...
    // 充值
//    @PutMapping("/user/recharge/{id}")
//    @Operation(summary = "充值")
//    public R recharge(@PathVariable("id") long id, @RequestParam double amount) {
//
//        User user = userService.getUserById(id);
